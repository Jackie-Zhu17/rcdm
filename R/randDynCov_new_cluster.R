#' Random Covariance Dynamic Model(cluster)
#'
...
randDynCov_cluster <- function(x, h, K, max.iter=1000, rho, lambda2, n0, ncore=8, k_list=NULL, G, parallel=FALSE) {
  ...
  cluster_list = list()
  for (g in 1:G) {
    cluster_list[[g]] = seq(g, g+G*(K/G-1), by = G)
  }
  ...
  if (parallel == TRUE) {
    ...
    while(max(abs(Omega_k_list-Omega_k_list.old))>delta | max(abs(Omega_t_list-Omega_t_list.old))>delta) {
      ...
      for (g in 1:G) {
        tmp = 0
        for (k in cluster_list[[g]]) {
          tmp = tmp + apply(sapply(1:T_count, function(t){w_h_k_t[k,t]*Omega_t_list[,,t]}, simplify = 'array'),
                            1:2,
                            sum)
        }
        Omega_k_list[,,cluster_list[[g]]] = tmp/sum(rowsum_w[cluster_list[[g]]])
      }
